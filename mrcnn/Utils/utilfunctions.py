...
def compute_backbone_shapes(config, image_shape):
    ...
    assert config.BACKBONE in ["resnet50", "resnet101", "resnet101V2"]
    return np.array(
...
def mask_polygonal(mask, eps=0.0101):
    """ Make mask with appearance polygonal.

    Params:
    - mask: [height, width] of type bool. A small, typically 28x28 mask.
    - eps: Epsilon value used in points approximation

    Returns: [height, width] of type bool. A small, typically 28x28 mask.
...
def modify_mask(mask, modifier):
    if modifier == "polygon":
        return mask_polygonal(mask)
...
def unmold_mask(
    ...
    y1, x1, y2, x2 = bbox
    if use_skimage_resize:
        mask = resize_skimage(mask, (y2 - y1, x2 - x1), order=skimage_resize_order)
    else:
        mask = resize(mask, (y2 - y1, x2 - x1), interpolation_method=interpolation_method)
    ...
    full_mask = modify_mask(full_mask, mask_modifier)
    return full_mask
...
def get_root_dir_path(root_dir='mrcnn'):
    current_dir = os.getcwd()
    if current_dir.split('/')[-1] == root_dir:
        return current_dir

    root_path = "/" if not os.system('uname -r') else ""
    for directory in current_dir.split('/'):
...
def download_dataset(dataset_name: str, dataset_url: str, download_path: str = ""):
    if not download_path:
        download_path = get_root_dir_path() + '/downloaded_datasets'
        if not os.path.exists(download_path):
            os.system("mkdir '{}'".format(download_path))
        download_path += '/' + dataset_name

    with urllib.request.urlopen(dataset_url) as resp, open(download_path, 'wb') as f:
        shutil.copyfileobj(resp, f)
    return download_path


def unzip_dataset(dataset_path: str):
    import time

    split_path = dataset_path.split('/')
    download_dir = '/'.join(split_path[:len(split_path) - 1])
    dataset_name = split_path[-1].split('.')[0]
    unzipped_path_dataset = os.path.join(download_dir, dataset_name)
    with zipfile.ZipFile(dataset_path, 'r') as zip_ref:
        zip_ref.extractall(unzipped_path_dataset)

    while not os.path.exists(unzipped_path_dataset):
        time.sleep(1)
